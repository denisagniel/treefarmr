...
\item \textbf{Related work, sharpened.}
  ...
  \item \citet{waltherzhao2026}'s Beta-trees: closest venue/theme cousin,
    ...
    the data, splits sit at empirical medians rather than loss-optimized
...
\item Lemma~\ref{lem:ancestor-localisation}: superconsistency survives
  ...
  the engine that makes the level-by-level, root-first induction over the
...
\begin{enumerate}[label=(\roman*),leftmargin=*]
  ...
  \]
  A cell with \(\operatorname{Leb}_{p-1}(G_j)=0\) (a source--target leaf
  pair whose footprints do not overlap) has \(\omega_{N,j}\equiv0\) and
  ...
  footprint has \(\omega_{N,j}(c)=\Theta(n^{-1})\).
...
only the one currently being refined, within $C/r_n$ of its true split.
