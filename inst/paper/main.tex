...
\item \S\ref{sec:setup-twostage}: the two-stage extension --
  Assumption~\ref{ass:sparsity-jump} (continuum tree sparsity + jump
  regularity) states the actual target class: a \emph{continuum}-threshold
...
\item \S\ref{sec:setup-solver}: Assumption~\ref{ass:solver-twostage} --
  ...
  (bracket containment, etc.) for the guarantees below to apply.
\end{itemize}]
...
convenience, e.g.\ Assumption~\ref{ass:sparsity-jump}'s $R_N$; this changes
nothing that follows, since adjacent leaves' cells share only a
$\nu$-null boundary face under Assumption~\ref{ass:sparsity-jump}(Dens).)
...
supplied by the analyst (Assumption~\ref{ass:sparsity-jump}(S) below requires
...
$\cS^{\mathrm{cont}}$, and \textbf{Assumption~\ref{ass:sparsity-jump}(S) is
strengthened, for the remainder of this paper, to assert that all minimal
...
(Assumption~\ref{ass:sparsity-jump}(S)) refers to the minimal-depth
...
\begin{enumerate}[label=(\Alph*),leftmargin=*]
  ...
  and depth at most \(\bar D\), where \(\bar L\) and \(\bar D\) are fixed in \(n\).
  ...
  coordinate would in any case violate the \(\omega_{N,j}(c)=\Theta(\abs{c-c_0})\)
  ...
  population values \(\gamma^-_{N,j}\) and \(\gamma^+_{N,j}\) satisfy
...
\begin{enumerate}[label=(\alph*),nosep]
  ...
  \item \emph{Valid brackets.} For every node $N$ of $\topolcoll(\hat t^A)$,
  the bracket $\widehat B_n(N)$ of Definition~\ref{def:stageb-bracket}
  contains the corresponding true threshold with probability tending to one.
...
Definition~\ref{def:stageb-bracket}, the endpoints of $\widehat B_n(N)$ are
themselves recovered splits within $C/r_n$ of distinct true boundaries, which
(Sep) separates by at least $\varsigma$. No rate condition linking $r_n$ to
$n$ is needed beyond the conditions already required by
Proposition~\ref{prop:boundary-recovery}. Stage A supplies only the topology
and the brackets $\widehat B_n(\cdot)$; every threshold value in the final
estimator is supplied by the off-grid Stage-B scan of
Algorithm~\ref{alg:twostage}, Step~4.
\end{remark}
...
\item Tie to the companion causal-inference paper (``Paper B''): this
  ...
  paper).
\item Limitations: fixed $\bar L$/depth (Assumption~\ref{ass:sparsity-jump}
  (S)); the classification-loss extension of the two-stage algorithm is an
