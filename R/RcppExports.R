# Generated by using Rcpp::compileAttributes() -> do not edit by hand
# Generator token: 10BE3573-1514-4C36-9D1C-5A225CD40393

treefarms_configure_cpp <- function(configuration) {
    invisible(.Call(`_optimaltrees_treefarms_configure_cpp`, configuration))
}

treefarms_fit_cpp <- function(data_csv) {
    .Call(`_optimaltrees_treefarms_fit_cpp`, data_csv)
}

treefarms_time_cpp <- function() {
    .Call(`_optimaltrees_treefarms_time_cpp`)
}

treefarms_iterations_cpp <- function() {
    .Call(`_optimaltrees_treefarms_iterations_cpp`)
}

treefarms_size_cpp <- function() {
    .Call(`_optimaltrees_treefarms_size_cpp`)
}

treefarms_status_cpp <- function() {
    .Call(`_optimaltrees_treefarms_status_cpp`)
}

treefarms_fit_with_config_cpp <- function(data_csv, configuration) {
    .Call(`_optimaltrees_treefarms_fit_with_config_cpp`, data_csv, configuration)
}

treefarms_fit_and_stats_cpp <- function(data_csv, configuration) {
    .Call(`_optimaltrees_treefarms_fit_and_stats_cpp`, data_csv, configuration)
}

cleanup_static_state <- function() {
    invisible(.Call(`_optimaltrees_cleanup_static_state`))
}

