...
public:    
    ...
    // @param sample: bitmask of binary features (encoded) used to make the prediction
    // @modifies probabilities: vector of probabilities for each class
    void predict_proba(Bitmask const & sample, std::vector<float> & probabilities) const;
...
private:
    ...
    std::vector<float> class_distribution; // class distribution for probability predictions
};
