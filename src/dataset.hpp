...
public:
    ...
    std::vector< double > const & get_target_values() const { return target_values; }
...
private:
    ...
    std::vector< double > target_values; // Regression: continuous y, one per sample (when loss == SQUARED_ERROR)
